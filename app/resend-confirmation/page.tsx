  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-4 py-8">
      <section className="w-full max-w-md rounded-lg border border-ink/10 bg-white p-6 shadow-soft">
        ...
        <h1 className="text-2xl font-semibold text-ink">Resend confirmation</h1>
        <p className="mt-2 text-sm text-ink/62">Enter the email you used to sign up and we&apos;ll send another confirmation link.</p>

        {message ? <p className="mt-4 rounded-md bg-saffron/10 px-3 py-2 text-sm text-ink/75">{message}</p> : null}

        <form action={resendConfirmation} className="mt-6 space-y-4">
